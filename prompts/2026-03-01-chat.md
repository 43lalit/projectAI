...
## Addendum - UI Guide Tour and Onboarding (2026-03-01, later session)
- Replaced header video-based watch tour with an in-app `Guide Me` onboarding flow.
- Added first-time auto-launch onboarding and manual relaunch from header button.
- Implemented overlay spotlight with contextual step card, navigation controls (`Back`, `Next`, `Skip`), and escape/outside-click close behavior.
- Added smooth guide transitions:
  - shade fade-in/fade-out
  - card fade/slide transitions
  - spotlight position transitions between targets
- Added onboarding intro step before navigation steps explaining product purpose and workspace scope.
- Added targeted navigation steps covering:
  - Discovery tab
  - Search input
  - Bookmarks tab
  - Reporting Viewer tab
  - Load MDRM tab
  - Theme toggle button
  - Profile/auth menu
- Added professional full-body female guide avatar support with static expression transitions per step.
- Integrated custom avatar file support (`mentor-custom.webp`) under `src/main/resources/static/guide/`.
- Recent Items UX refinement:
  - hide panel when empty (including first load)
  - hide panel immediately after clear action
  - show panel only when discovery view is active and recent items exist

### Files Updated (session)
- `src/main/resources/static/index.html`
- `src/main/resources/static/css/mdrm-console.css`
- `src/main/resources/static/guide/mentor-custom.webp`
- `README.md`
